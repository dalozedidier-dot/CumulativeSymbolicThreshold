...
def _analyze_run(
    ...
    df_test.iloc[pre_start:pre_end]["C"].to_numpy(dtype=float)
...
def _generate_report_md(
    ...
    lines += [
        ...
        "- **Conditions**: TEST (demand_shock), STABLE (no intervention), PLACEBO (shifted intervention)",
        f"- **Total runs**: {n_replicates * 3}",
        "- **Parameters**: frozen ex ante (see contracts/FROZEN_PARAMS.json)",
        "",
        "## Confusion Matrix",
        "",
        "```",
    ...
    lines += [
        ...
        "",
        "---",
        "Generated by: run_scientific_validation_protocol.py",
        f"Output: {outdir}",
    ]
