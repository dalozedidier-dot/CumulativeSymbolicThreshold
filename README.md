...
> - **Synthetic T1–T8:** indicative — the only stored `full_statistical` proof run with the obligatory triplet (p + CI₉₉ % + SESOI + power) is the endogenous bistable *demonstration* (`05_Results/endogenous_full_statistical/`); no **real-data pilot** has one yet.
...
> - **Every claimed rung is now backed by a committed artifact** ([`docs/EVIDENCE_LADDER.md`](docs/EVIDENCE_LADDER.md)): strong negatives `STRONG_NEGATIVES_CLEAN` (`05_Results/strong_negatives/`), registered FRED A/B/C block `REAL_BLOCK_REJECTED` — an honest, *specific* negative (`05_Results/registered_block/`), and the first confirmatory pre-registered OOS run under the frozen `lead=60` protocol → `OOS_SKILL_INCONCLUSIVE` (`05_Results/oos_prediction/`). Anti-drift tests fail CI if a stored artifact deviates from its frozen contract.
