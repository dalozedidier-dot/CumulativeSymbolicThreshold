...
Architecture, métriques, mécanisme, test
...
Par défaut, C(O,R,I) est une capacité synergique, par exemple C(O,R,I) = O·R·I ou une moyenne géométrique pondérée.
La forme exacte est fixée avant test.
...
Cette rétroaction modifie les attracteurs du cycle O, R, I : elle change ce qui se stabilise (O), ce qui amortit (R), et ce qui arbitre (I).
Ce déplacement des régimes internes rend le basculement observable et falsifiable.
...
La variation exploratoire modulable (plasticité, apprentissage) joue le rôle de moteur de reconfiguration.
La variation génétique fournit un fond de nouveauté à long terme, sans agir comme levier instantané.

## 2) Variable d’ordre et basculement

### Variable d’ordre C(t)
C(t) n’est pas la culture au sens vague.
C’est une métrique opérationnelle.
...
s(t) = ΔV(t) / ΔS(t), mesuré sous intervention, sur la fenêtre [t−Δ,t][t-\Delta,t][t−Δ,t] et avec protocole déclaré.
...
Le test exige que leur perturbation dégrade la viabilité.
L’intervention peut être expérimentale, simulationnelle, ou quasi expérimentale par contrefactuel instrumenté.
