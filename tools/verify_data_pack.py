...
def _git_toplevel_matches_root() -> bool:
    ...
    Source archives are often extracted inside a parent Git checkout during CI.
    Without this guard, ``git ls-files`` can silently inspect the parent
    repository instead of the extracted archive.
    """
    try:
        out = subprocess.run(
